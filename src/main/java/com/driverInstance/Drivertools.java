...
//	private String bundleId;
	...
	public void setOS(String getOs) {
		this.getOs = getOs;
	}
...
//	protected void setAppBundleId(String bundleId) {
...
//	}
	...
	public Drivertools(String application,String deviceName,String portno) {
		...
		setOS(getHandler().getproperty("OS"));
...
//		setremoteUrl("http://" + getHost() + ":" + portno);
...
//		setAppBundleId(getHandler().getproperty("bundleId"));
	...
	{		
		...
		if (getPlatform().equals("Web")) {
			...
			setENV("Native App");
			click = false;
		} else if (getPlatform().equals("MPWAiOSSafari")) {
			setENV("Safari Application");
			click = false;
		}
		
		logger.info("PlatForm :: " + getPlatform());
		if (Stream.of("Android", "iOS", "Web", "MPWA","BrowserStack","IOSBrowserStack","MPWAiOSSafari").anyMatch(getPlatform()::equals)) {

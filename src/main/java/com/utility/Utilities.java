...
//	final static Logger logger = Logger.getLogger("rootLogger");
	...
	public static void initDriver() {
		...
		} else if (getPlatform().equals("Android") || getPlatform().equals("MPWA")|| getPlatform().equals("BrowserStack") || getPlatform().equals("IOSBrowserStack")) {
...
//			e.printStackTrace();
	...
	public static WebElement findElement(By byLocator) throws Exception {
		...
		if (getPlatform().equals("Web")) {
			...
			return element;
		} else if (getPlatform().equals("MPWA") || getPlatform().equals("Android")) {
...
//			logger.error(e);
	...
	public static void type(By byLocator, String input, String FieldName) throws Exception {
		try {
			//String platform = Reporter.getCurrentTestResult().getTestContext().getCurrentXmlTest().getSuite().getName();
			if (platform.equalsIgnoreCase("Android") || platform.equalsIgnoreCase("BrowserStack")
					|| platform.equalsIgnoreCase("IOSBrowserStack") || platform.equalsIgnoreCase("iOS") || platform.equalsIgnoreCase("MPWAiOSSafari")) {
...
//	public static void assertionValidation(String actual, String expected, String validationText,String validationType) throws Exception {
//		try{
//			if ("contains".equals(validationType)) {
//				softAssert.assertEquals(actual.contains(expected), true);
//				if (actual.equals(expected)) {
//					logger.info(actual + " and " + expected + " "+validationText+" are matched");
//					ExtentReporter.extentLoggerPass("Assertion", actual + " and " + expected + " "+validationText+" are matched");
//				}
//			} else if ("equals".equals(validationType)) {
...
//			}
//		}catch(Exception e){
//			logger.info(actual + " "+validationText+" doesn't "+validationType+ " " + expected);
//			ExtentReporter.extentLoggerFail("Assertion", actual + " "+validationText+" doesn't "+validationType+ " " + expected);
...
//    public static String getURLBrowserName() throws Exception {
//  	  String browserName = ((RemoteWebDriver) DriverManager.getDriver()).getCapabilities().getBrowserName().toLowerCase();
//  	  String url = ((RemoteWebDriver) DriverManager.getDriver()).getCurrentUrl();
//        System.out.println("Browser Name is : " + browserName + " " + url);
...
//			logger.error("Not scrolled to element " + message);
//			ExtentReporter.extentLoggerFail("ScrollToElement", "Not scrolled to element " + message);
		}
	...
	public static boolean verifyElementNotDisplayed(By byLocator, String str) throws Exception {
		...
		try {
			if(getPlatform().equalsIgnoreCase("web")) {
				...
				element = DriverManager.getAppiumDriver().findElement(byLocator);
				explicitWaitVisibility(byLocator, 5);
				if (element.isDisplayed()) {
					softAssert.assertEquals(element.isDisplayed(), true, "" + str + " " + " is displayed");
					ExtentReporter.extentLogger("checkElementPresent", "" + str + " is displayed");
					logger.info("" + str + " is displayed");
...
//			softAssert.assertEquals(true, false, str + " " + " is not displayed");
			ExtentReporter.extentLogger("checkElementPresent", "" + str + " is not displayed");
			logger.info(str + " is not displayed");
			return true;
		}
	...
	public static String generateRandomEmail() {
		...
		String domain = "gmail.com"; // Set the domain

		return randomString.toString() + "@" + domain;
	}
	/**
	 * TMO Login from Login Page
	 */
	/**
	...
	public static boolean tapOnScreenUsingBounds(int leftX, int topY, int rightX, int bottomY, String message) {
		AppiumDriver driver = DriverManager.getAppiumDriver();
		boolean flag = true;
		// Given bounds [176, 1402][808, 1523]
		leftX = 176;
		topY = 1402;
		rightX = 808;
		bottomY = 1523;

		// Calculate the center coordinates within the bounds
		int centerX = (leftX + rightX) / 2;
		int centerY = (topY + bottomY) / 2;

		// Create a TouchAction
		TouchAction touchAction = new TouchAction(driver);

		// Perform a tap action at the calculated center coordinates
	...
	public static int GetCoordinatesYForFirstProfileIcon() throws Exception {
		int y=0;
		if(getParameterFromXML("deviceName").equals("RZCW20MGVVK")){
			y=852;
		} else if(getParameterFromXML("deviceName").equals("ZY326L686M")){
	...
	public static int GetCoordinatesXForUserIconInMenu() throws Exception {
		int x=0;
		if(getParameterFromXML("deviceName").equals("RZCW20MGVVK")){
			x=561;
		} else if(getParameterFromXML("deviceName").equals("ZY326L686M")){
	...
	public static int GetCoordinatesYForUserIconInMenu() throws Exception {
		int y=0;
		if(getParameterFromXML("deviceName").equals("RZCW20MGVVK")){
			y=641;
		} else if(getParameterFromXML("deviceName").equals("ZY326L686M")){
	...
	public static int GetCoordinatesXForAccount() throws Exception {
		int x=0;
		if(getParameterFromXML("deviceName").equals("RZCW20MGVVK")){
			x=540;
		} else if(getParameterFromXML("deviceName").equals("ZY326L686M")){
	...
	public static int GetCoordinatesYForAccount() throws Exception {
		int y=0;
		if(getParameterFromXML("deviceName").equals("RZCW20MGVVK")){
			y=1034;
		} else if(getParameterFromXML("deviceName").equals("ZY326L686M")){
...
//=============================================================================================================//
	...
	public static void actionSendKeys(By byLocator,String input,String validationText) throws Exception {
		try{
			Actions a = new Actions(DriverManager.getAppiumDriver());
			WebElement ele = DriverManager.getAppiumDriver().findElement(byLocator);
	...
	public static String  propertyFileReaderFromProperties(String Data,String Module) {
		String osName=System.getProperty("os.name").toLowerCase();
		System.out.println(osName);
		PropertyFileReader prop;
		String KeyValue;
	...
	public static void relaunchApp( String platform) {
		try {
			switch (platform.toLowerCase()) {
				case "android": {
					DriverManager.getAppiumDriver().activateApp(propertyFileReaderFromProperties("AndroidPackage","TONIK"));
...
//             driver.activateApp();
	...
	public static void updateSignature(){
		...
		WebElement element = DriverManager.getAppiumDriver().findElement(By.xpath("(//android.widget.LinearLayout)[3]/child::*"));
		touchAction.longPress(ElementOption.element(element)).waitAction(WaitOptions.waitOptions(Duration.ofMillis(1000)))
				.moveTo(PointOption.point(682, 1068)).release().perform();
	}
	 ...
	 * @throws Exception
	 */
	public static void enterOTP() throws Exception {
